...
export default function SetupForm() {
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const body=await response.json().catch(()=>({error:"The server could not complete setup."}));
      if(!response.ok){setError(body.error);setBusy(false);return} window.location.assign("/calculator");
    } catch {
      setError("Setup could not be completed. Please try again."); setBusy(false);
    }
  }
